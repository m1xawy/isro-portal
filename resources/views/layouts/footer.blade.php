@php
    $backlinks = cache()->remember('backlinks', setting('cache_setting', 600), function() { return json_decode(setting('backlinks')); });
    $socials = cache()->remember('socials', setting('cache_setting', 600), function() { return json_decode(setting('socials')); });
@endphp
    ...
    <div class="mx-auto w-full max-w-screen-xl p-4 py-6 lg:py-8">
        <div class="md:flex md:justify-between">
            <div class="mb-6 md:mb-0">
                <a href="{{ $server_url }}" class="flex items-center">
                    @if (!empty($server_logo))
                        <img src="{{ asset(Storage::url($server_logo)) }}" class="w-40 mr-3" alt="{{ $server_name }}"/>
                    @else
        ...
        <div class="sm:flex sm:items-center sm:justify-between">
            <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">© 2023 <a href="{{ $server_url }}" class="hover:underline">{{ $server_name }}</a>. All Rights Reserved. · Coded by <a class="link-default" href="https://mix-shop.tech/">m1xawy</a></span>
